...
- 'features.txt': List of all features.
The column names in the file could not be referenced by column name in a dataframe, a name such as "tBodyAcc-mean()-X" 
is not syntactically coorrect.  The use of the function make.names will make the name syntactically valid, eg., the variable  "tBodyAcc-mean()-X" was renamed to "tBodyAcc.mean...X".  Below is the new names provide by make.names():

```sh
features <- read.table("./UCI HAR Dataset/features.txt",header=FALSE,colClasses="character")
mn <- make.names(features$V2)
```
...
### Read test data ###
The column names are automatically assigned as shown. The col.name=mn is the resultant column names from make.names() function as previously mentioned. 
...
### Reshape<sup>1</sup> the data to get the average of each variable for each activity and each subject.  ###
...
The resultant output file, tidy_data.txt, is located in your current working directory.



####  References ####
<sup>1</sup> Cookbook for R: Converting data between wide and long format                                                                 http://www.cookbook-r.com/Manipulating_data/Converting_data_between_wide_and_long_format/
